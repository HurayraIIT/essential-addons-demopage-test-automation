...
// Content Style & Content Preset Combinations Start
  ...
  test("Content Style: Basic | Content Preset: Preset 1", async ({ page }) => {
    ...
    // Button
    await expect(basicPreset1Locator.getByRole("link", {name: /Explore BasicPreset1/})).toBeVisible();
    await expect(basicPreset1Locator.getByRole("link", {name: /Explore BasicPreset1/})).toBeEnabled();
    // await expect(basicPreset1Locator.getByRole("link", {name: /Explore More/})).toHaveURL("https://eael.site");
    await expect(basicPreset1Locator.getByRole("link", {name: /Explore BasicPreset1/})).toHaveClass("cta-button  effect-2");
    await expect(basicPreset1Locator.getByRole("link", {name: /Explore BasicPreset1/})).toHaveCSS("box-shadow", "rgba(27, 33, 45, 0.72) 4px 4px 0px 0px");
    await expect(basicPreset1Locator.getByRole("link", {name: /Explore BasicPreset1/})).toHaveCSS("font-size", "19px");
    await expect(basicPreset1Locator.getByRole("link", {name: /Explore BasicPreset1/})).toHaveCSS("font-family", "Poppins, sans-serif");
  });
});
