...
test.describe("Countdown", () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    await expect(page.getByRole("link", { name: "Documentation" })).toHaveAttribute(
      "href",
      /docs\/creative-elements\/ea-countdown/
    );
  });
  ...
  test("Test Section: Showcase Stunning Countdown Effects", async ({ page }) => {
    ...
    await expect(
      page.locator("li:nth-child(2) > .eael-countdown-hours > .eael-countdown-digits").first()
    ).toBeVisible();
    await expect(page.locator("li:nth-child(2) > .eael-countdown-hours > .eael-countdown-label").first()).toBeVisible();
    await expect(
      page.locator("li:nth-child(3) > .eael-countdown-minutes > .eael-countdown-digits").first()
    ).toBeVisible();
    await expect(
      page.locator("li:nth-child(3) > .eael-countdown-minutes > .eael-countdown-label").first()
    ).toBeVisible();
    await expect(
      page.locator("li:nth-child(4) > .eael-countdown-seconds > .eael-countdown-digits").first()
    ).toBeVisible();
    await expect(
      page.locator("li:nth-child(4) > .eael-countdown-seconds > .eael-countdown-label").first()
    ).toBeVisible();
  });
});